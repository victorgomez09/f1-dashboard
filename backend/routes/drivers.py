...
async def get_standings_evolution(year: int):
    try:
        ...
        schedule["Session5DateUtc"] = pd.to_datetime(
            schedule["Session5DateUtc"], utc=True
        )

        past_events = schedule[
            (schedule["EventFormat"] != "testing") & (schedule["Session5DateUtc"] < now)
        ]
        ...
        for _, event in past_events.iterrows():
            ...
            try:
                ...
                for _, row in results.iterrows():
                    ...
                    # Acumular
                    acc_points_drivers[d_code] = (
                        acc_points_drivers.get(d_code, 0) + points
                    )
                    acc_points_constructors[team_name] = (
                        acc_points_constructors.get(team_name, 0) + points
                    )
        ...
        for _, event in past_events.iterrows():
            ...
            try:
                ...
                sorted_drivers = sorted(
                    acc_pts_drivers.items(), key=lambda x: x[1], reverse=True
                )
                rank_drivers = {
                    code: i + 1 for i, (code, pts) in enumerate(sorted_drivers)
                }
                ...
                for d_code, evo in drivers_map.items():
                    evo["rounds"].append(
                        {
                            "round": rnd,
                            "position": rank_drivers.get(
                                d_code, len(rank_drivers)
                            ),  # Su posición en el mundial
                ...
                sorted_teams = sorted(
                    acc_pts_constructors.items(), key=lambda x: x[1], reverse=True
                )
        ...
        for _, event in past_events.iterrows():
            ...
            try:
                ...
                for _, row in res.iterrows():
                    ...
                    pts_acc_constructors[c_id] = (
                        pts_acc_constructors.get(c_id, 0) + pts_race
                    )
                    ...
                    drivers_map[d_code]["rounds"].append(
                        {
                            ...
                            "position": int(
                                row["Position"]
                            ),  # Posición en esta carrera
                            "points": pts_acc_drivers[
                                d_code
                            ],  # Puntos totales hasta ahora
                            "locality": loc,
                        }
                    )
    ...
    try:
        ...
        for _, event in past_events.iterrows():
            ...
            try:
                ...
                results = session.results.drop_duplicates(
                    subset=["Abbreviation"], keep="first"
                )
                ...
                for _, row in results.iterrows():
                    ...
                    is_dns = any(
                        msg in status
                        for msg in ["Did not start", "DNS", "Withdrawn", "107%"]
                    )
        ...
        final_data.sort(
            key=lambda x: (x["Wins"], x["Podiums"], x["PointsFinish"]), reverse=True
        )
    ...
    try:
        ...
        for _, event in past_events.iterrows():
            ...
            try:
                ...
                round_entry = {"name": r_name, "locality": r_locality}
...
@router.get("/{year}")
def get_drivers_profiles(year: int):
    try:
        file_path = os.path.join(RESULTS_DIR, f"drivers_info_{year}.json")
        if os.path.exists(file_path):
            with open(file_path, "r") as f:
                return json.load(f)
            
        schedule = fastf1.get_event_schedule(year)
        past_events = schedule[schedule["EventFormat"] != "testing"]
        if past_events.empty:
            raise HTTPException(status_code=404, detail="No events found for this year")

        last_event = past_events.iloc[-1]
        session = fastf1.get_session(year, last_event["RoundNumber"], "R")
        session.load(laps=False, telemetry=False, weather=False)
        ...
        for _, row in session.results.iterrows():
            ...
            if not headshot_url or headshot_url == "None":
                first_name = row["FirstName"]  # Ejemplo: "Kimi" o "Andrea Kimi"
                last_name = row["LastName"]  # Ejemplo: "Antonelli"
                primary_name = first_name.split(" ")[0]
                clean_primary = re.sub(r"[^A-Z]", "", primary_name.upper())
                clean_last = re.sub(r"[^A-Z]", "", last_name.upper())

                ref_code = f"{clean_primary[:3]}{clean_last[:3]}01"
                initial = primary_name[0].upper()

                folder_slug = f"{ref_code}_{first_name}_{last_name.replace(' ', '_')}"
                encoded_folder = urllib.parse.quote(folder_slug)

                headshot_url = (
                    f"https://media.formula1.com/d_driver_fallback_image.png/"
                    f"content/dam/fom-website/drivers/{initial}/{encoded_folder}/"
                    f"{ref_code.lower()}.png.transform/1col/image.png"
                )

            team_raw = row["TeamName"]
            noise = ["f1", "team"]

            parts = re.split(r"[\s-]+", team_raw.lower())
            clean_parts = [p for p in parts if p not in noise and len(p) > 1]
            slug = "-".join(clean_parts) if clean_parts else parts[0]

            team_logo_url = f"https://media.formula1.com/content/dam/fom-website/teams/{year}/{slug}-logo.png"
            ...
            profile = {
                ...
                "team_color": f"#{row['TeamColor']}" if row["TeamColor"] else "#FFFFFF",
                "country_code": row["CountryCode"],
                "headshot_url": headshot_url,
                "team_logo_url": team_logo_url,
            }
            drivers_data.append(profile)
