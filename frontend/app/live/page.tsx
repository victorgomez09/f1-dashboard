...
export default function LiveDashboard({ location, year, sessionType }: { location: string, year: number, sessionType: string }) {
    const { map: trackData, isLoading: isMapLoading } = useMap(year, location, sessionType);
    const { timing, mapData, messages, radios } = useLiveTiming();
        ...
        <main className="grid grid-cols-12 gap-2 p-2 overflow-hidden w-full h-full">
            ...
            <section className="col-span-12 lg:col-span-5 flex flex-col gap-2 overflow-hidden">
                ...
                <div className="flex-1 bg-base-300 rounded-box relative min-h-100">
                    <TrackMap mapData={mapData} trackPoints={trackData?.data || []} />
                </div>
